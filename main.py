...
print "High pixel gap means low accuracy but low computational cost and vice-versa.\n"
print "Time-interval-> Time interval between adjusting your screen brightness.\n"
print "Sensitivity-> Set sensitivity of change in pixels for which brightness changes"
print "Low sensitivity would change brightness on larger change in screen colour and vice versa \n"
...
class Window(QtGui.QMainWindow):
    ...
    def __init__(self):
        ...
        l5.setText("Note: Closing this will close the application, Rather minimize to System Tray")
...
# adjust_brightness calculate the current state of the screen and adjusts brightness accordingly
    ...
    while 1:
        ...
        for x in range (0,MAX_X,increment):
            for y in range (0,MAX_Y,increment):
                ...
                G=rgb[1]
                B=rgb[2]
                brightness=math.sqrt(0.299*R*R + 0.587*G*G + 0.114*B*B )            #formula to derive brightness of a pixel from RGB
                sum_brightness+=brightness

        average_brightness=sum_brightness/MAX_X/MAX_Y*increment*increment           #average value of brightness
        difference=255-average_brightness
        mean=difference/255
        sensitivity_diff = 100 - sensitivity
        sensitivity_mean =float(sensitivity_diff)/1000

        if(float(mean+0.3)>float(brightness_set + sensitivity_mean) or float(mean+0.3)<float(brightness_set-sensitivity_mean)):
